...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float4 col, float2 texCoord, float rotation, float2 rotCosSin, float2 offset, float2 pivotOffset)
{
	if (rotation != 0)
	{
		...
		//Transform to origin
		float3 tempPos = pos - float3(offset + pivotOffset, 0);
		//Rotate
...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
	...
	float2 textureSize = gTextureSize * scale;
	...
	texCoord = float2(0, 0);
	//VERTEX 1 [LT]
	CreateVertex(triStream, position, float4(1,1,1,1), texCoord, rotation, rotCosSin, offset, pivotOffset);
