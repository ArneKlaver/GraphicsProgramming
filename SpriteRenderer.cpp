...
void SpriteRenderer::UpdateBuffer(const GameContext& gameContext)
{
	...
	if (m_Sprites.size() > m_BufferSize || m_pVertexBuffer == NULL)
	{
		//release the buffer
		SafeRelease(m_pVertexBuffer);
		...
		bufferDesc.ByteWidth = m_Sprites.size() * sizeof(SpriteVertex);
		...
		gameContext.pDevice->CreateBuffer(&bufferDesc, NULL, &m_pVertexBuffer);
	}

	D3D11_MAPPED_SUBRESOURCE subresource;
	gameContext.pDeviceContext->Map(m_pVertexBuffer, 0, D3D11_MAP::D3D11_MAP_WRITE_NO_OVERWRITE, 0, &subresource);
	memcpy(subresource.pData, &m_Sprites[0], m_BufferSize * sizeof(SpriteVertex));
